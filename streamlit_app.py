...
# 年齢スライダー
age = st.slider("あなたの年齢を教えてください", 0, 80, 20)
...
if 0 <= age <= 1:
    ...
    if st.button("診断する"):
        ...
        if yes_count >= 7:
            st.success("あなたは「世界は信頼できる場所だ」という感覚がしっかり根づいているようです。対人関係でも自分を守る安心感が土台にあり、感情やニーズを素直に出すことができているかもしれません。")
        elif 4 <= yes_count <= 6:
            st.info("幼少期に安心感を得られた場面と、そうでない場面が混在していたかもしれません。人を信じたいけれど怖さが勝つ時がある、そんな感覚も自然なことです。少しずつ「信頼しても大丈夫だった」経験を増やしていきましょう。")
        else:
            st.warning("安心して誰かに頼ることが難しかった経験や、見捨てられる不安を抱えやすい傾向があるかもしれません。でもそれはあなたのせいではなく、育った環境の中で守られきれなかっただけ。いまから「安心を知る経験」を積むことは、いつからでも始められます。")
...
# 青年期の範囲のみ表示
    ...
    if st.button("診断する"):
        ...
        if yes_count >= 4:
            st.success("あなたは今、**自己理解と他者との関係の中で深く模索する時期**にいます。その悩みや違和感は、成長の証です。安心して、今の自分と対話を続けてください。")
        elif 2 <= yes_count < 4:
            st.info("あなたは今、**少しずつ自分らしさを掴み始めている段階**です。モヤモヤは悪いことではなく、“気づき”のきっかけ。焦らず進んで大丈夫。")
        else:
            st.warning("今はあまり違和感を感じていないかもしれません。でも、自分に問いかける時間をつくることで、今後の指針がクリアになるかもしれません。")
